...
Some other input partitioning are: **invalid inputs**(to check if a system can handle invalid inputs), **special cases**(if system can handle special values like NaN values), and **combination of inputs**(for a comprehensive test suite).
