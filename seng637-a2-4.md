...
`testCombineWithOverlappingRanges`:

Covers scenarios where two ranges overlap partially.
Partitions Covered: Equivalence Partitioning, Boundary Value Analysis.
`testCombineWithNoOverlapInRanges`:

Covers scenarios where two ranges have no overlap.
Partitions Covered: Equivalence Partitioning.
`testCombineWithIdenticalRanges`:

Verifies that combining identical ranges returns the same range.
Partitions Covered: Equivalence Partitioning.
`testCombineWithRangeBoundsDoubleLimits`:

Verifies the behavior of the combine method with range bounds at the limits of Double.
Partitions Covered: Boundary Value Analysis.
`testCombineWithLargeRanges`:

Covers scenarios where very large ranges are combined.
Partitions Covered: Equivalence Partitioning, Boundary Value Analysis.
`testCombineWithNaNValues`:

Covers scenarios where ranges contain NaN values.
Partitions Covered: Equivalence Partitioning.
`testCombineWithNullRange`:

Verifies correct behavior when one range provided to the combine method is null.
Partitions Covered: Equivalence Partitioning.
`testCombineWithBothRangesNull`:

Verifies correct behavior when both ranges provided to the combine method are null.
Partitions Covered: Equivalence Partitioning.
